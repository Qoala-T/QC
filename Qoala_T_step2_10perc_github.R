...
# This package contains data and R code for step 2B as part of our Qoala-T tool:
#   
...
# load Rating,this file should include one column with rating and one column with visitID
rating_testdata <- data.frame(read.table("rating.txt",sep="\t",header=TRUE))
dataset <- merge(rating_testdata,rating_testdata,by.x="MRI_ID",by.y="row.names",all.y=T)
